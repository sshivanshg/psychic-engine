## Who you are (every session, every change)
  ...
  **Keep quant core pure and LLM layer optional/degradable. The API/frontend are READ layers — no
  ...
  - `context.py` / `sources.py` / `ingest.py` (also writes the **append-only `price_vintages` revision log** — reproducible back-tests across vendor re-adjustments) / `db.py` (**process-wide `psycopg_pool` connection pool; pgvector registered once per connection via the pool `configure` hook; lazy so import needs no live DB**) / `config.py` [OK]   ·   `docs.py` [OK] RAG (fastembed bge-small 384d → pgvector → CitedAnswer); **`list_documents` = browse a symbol's ingested corpus (source/period/filing_date, no embeddings)**   ·   `cache.py` [OK] **read-layer TTL memo (`READ_CACHE_TTL`, default 300s) returning deep copies — API-only, engine stays pure**
  - `analyst.py` [OK] **multi-angle single-name engine — `assemble_facts` (FREE: 6 analyzers + 6-quarter results trend + raw headlines + `events.py` catalyst tags) → tiny digest → ONE small Haiku bull/bear/judge `verdict` (~$0.003, degrades to a deterministic dial read w/o a key); descriptive-only, cites the provided number; nulls single-name portfolio-relative fields. ALSO `deep_analysis` (the "kachaa chitta" read: bull · bear · sector specialists in PARALLEL → a judge → structured `DeepAnalysis` — thesis, what's-right/wrong each citing a number, sector context, DESCRIPTIVE conditional scenarios, what-to-watch, bottom line; ~$0.05–0.10 on `DEEP_MODEL`=Sonnet, 4 calls, degrades to facts w/o a key, persisted to `analyst_runs`) · `ask_research` (follow-up over the WHOLE research: facts digest + latest persisted deep + RAG filing chunks + live `web_search`, inline-cited, descriptive, web/news look-ahead-barred for a past as_of)**   ·   `coverage.py` [OK] **`data_coverage` = per-symbol presence+freshness across every ingest table (prices/fundamentals/news/ownership/docs) — the blind-spot map; degrades holdings→universe**   ·   `sentiment.load_headlines` [OK] **raw title/publisher/polarity headlines (PIT-aware) for the news UI** (the dial only needs polarity)
  ...
  - **Phase 5:** `api.py` [OK] FastAPI read layer over the engine (`tradeos serve` / `tradeos-api`; `/api/*` incl. `/stock/{sym}/series` for charts; serves the built SPA); **factual analysis/risk/eval memoised via `cache.py` (portfolio + per-stock share one key); `/api/stock/{sym}` narrates ONLY that holding (`narrate_cards([card])`), not the whole book**; **thin WRITE SEAM (`POST /api/holdings`, `DELETE /api/holdings/{sym}`, `POST /api/docs` file upload, `POST /api/ingest`) — each calls the SAME `config`/`docs`/`ingest` function the CLI uses (I/O orchestration, no quant logic) then `cache.clear()`; upload validated to pdf/txt/md ≤25 MB into `data/uploads/`; no auth ⇒ 127.0.0.1 only**; **LIVE STREAM `GET /api/stream/analyze` (SSE) — runs `analyze(on_event=…)` fresh in a worker thread, drains a queue, streams every stage (run_start · context_loaded · agent_start/done · ranking · card · narration_* · run_complete), paced by `STREAM_PACING_MS` (140)**; **EVIDENCE LAYER (thin, reuse the engine) — `GET /api/analyst/{sym}?verdict=false|true` (the full single-name fact base free; `verdict=true` adds the Haiku read; `SystemExit` no-data → clean 404 via `_analyst_guard`), `GET /api/analyst/{sym}/deep` (the multi-agent deep read — facts + judge `DeepAnalysis` + raw bull/bear/sector debate + cost), `POST /api/analyst/ask` (ask-the-analyst over the whole research + live web; distinct from RAG-only `/api/ask`), `GET /api/stock/{sym}/news` (raw headlines + per-headline catalyst tag + flow summary), `GET /api/stock/{sym}/docs` (corpus browse), `GET /api/news` (portfolio-wide newsroom), `GET /api/coverage` (data-coverage matrix); `_sanitize` recursively JSON-ifies numpy/date/Pydantic; newsroom+coverage degrade holdings→universe**   ·   `briefing.py` [OK] pure alert-rule engine + pre-market briefing (`tradeos briefing`)   ·   `technical.price_series` [OK] point-in-time close + SMA20/50/200 + volume for charts
  - `web/` [OK] **SvelteKit (Svelte 5) dashboard with Apache ECharts** — sidebar layout; overview (KPI tiles · risk-contribution-vs-weight bars · sector donut · correlation heatmap · cards) · briefing · **manage (holdings add/remove · quarterly-results/transcript upload · doc-coverage table · refresh-data)** · **live Reasoning Monitor (`/live` — EventSource over `/api/stream/analyze`: watch the 6 agents run, each one's output, per-holding attention decomposition, streamed LLM synthesis, + a raw event tape; cards link into the Workbench)** · **EVIDENCE LAYER: Analyst Workbench (`/analyst/[symbol]` — sticky header (primary CTA = deep read) + 9 tabs: Overview · Technical · Fundamentals · News & Catalysts · Ownership · Risk · Documents · Reasoning · **Analysis**; surfaces every fetched detail; the **Analysis** tab runs the opt-in deep multi-agent read (what's-right/wrong · sector & how-it-should-perform scenarios · what-to-watch · bottom line · the raw bull/bear/sector agent debate) and hosts **Ask the analyst** (follow-up chat over the whole research + live web, with cited filing excerpts + web sources); the Documents tab keeps the RAG-only **Ask the filings**; History renders persisted deep reads; **now the sole per-symbol screen — the legacy `/stock/[symbol]` drill-in and the `/eval` signal page were removed 2026-06-01 as off-scope for the sense-making north star; the price/gauge/returns chart builders they used live on in the Workbench**), Newsroom (`/news` — all fetched headlines, filterable), Data Coverage (`/coverage` — blind-spot matrix), and a ⌘K command palette**. Reusable: `NewsFeed`/`QuarterTable`/`SentimentBar`/`ProvenanceBadge` (every panel badges `as_of` + snapshot-vs-PIT). `Chart.svelte` wraps ECharts; `lib/charts.ts` holds the option builders (+ `quarterlyTrendOption`/`ownershipDonutOption`/`polarityTimelineOption`), now **theme-aware (reads live CSS vars)**. **`lib/theme.ts` = black (default) / light theme store keyed on `<html data-theme>`, persisted, no-flash inline init; sidebar toggle; charts re-mount via `{#key $theme}`.**
  ...
  a SvelteKit SPA (`web/` — overview · Analyst Workbench · briefing),
